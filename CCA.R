...
#Call your data
#See the dataset in the main folder
setwd("/Users/faithravelo/Desktop/Trial") #Specify your path folder
...
cormat<-matcor(spe,env)
round(cormat$Ycor, 4)

#between set associations (correlation coefficient)
cormat<-matcor(spe,env)
...
#Set a scaling variable to multiply the CCA values, to get a very similar plot to the one generated by the plot(cca_model). You can adjust it according to your data
scaling_factor <- 2
...
combined_data <- cbind(spe, env)

#Fit a linear model for species1 using environmental variables
#Define species and environmental factors here 
lm_species1 <- lm(Pyrodinium ~ AP + Sand, data = combined_data)
summary(lm_species1)
